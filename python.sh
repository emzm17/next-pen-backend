...
run_python_and_capture_output() {
    if [ -f "$INPUT_FILE" ]; then
        python3 "$PYTHON_FILE" < "$INPUT_FILE" > "$FINAL_OUTPUT" 2>&1
    else
        python3 "$PYTHON_FILE" > "$FINAL_OUTPUT" 2>&1
    fi

    if [ $? -ne 0 ]; then
        echo "Execution failed. Printing error output..."
        cat "$FINAL_OUTPUT"  # Print error in readable format
    
        echo "Encoding error output..."
...
# Function to insert Base64-encoded content into Redis with flag
insert_into_redis() {
    local status_flag="$1"
...
# Execute function to run Python script and capture output
run_python_and_capture_output

# Convert final output to Base64 and store it
base64 "$FINAL_OUTPUT" > "$ENCODED_OUTPUT"

# Insert Base64-encoded output with "COMPLETED" flag
insert_into_redis "COMPLETED"

echo "Execution successful. Base64 output saved in $ENCODED_OUTPUT and published to Redis."
